...
namespace Teste_logica_f360
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> palavrasTextoA = textA.Split(' ').ToList();
            List<string> palavrasTextoB = textB.Split(' ').ToList();

            List<char> letrasFoo = new List<char> { 's', 'l', 'f', 'w', 'k' };
            List<char> alfaKlingon = new List<char> { 'k', 'b', 'w', 'r', 'q', 'd', 'n', 'f', 'x', 'j', 'm', 'l', 'v', 'h', 't', 'c', 'g', 'z', 'p', 's' };


            int preposicao = 0;
            int verbos = 0;
            int verbosPrimeiraPessoa = 0;
            List<string> vocabulario = new List<string>();
            int numBonitos = 0;


            foreach (string palavra in palavrasTextoB)
            {
                if (!vocabulario.Contains(palavra)) // adiciona as palavras desordenadamente no vocabulario
                {
                    vocabulario.Add(palavra);
                    vocabulario = vocabulario.OrderBy(i => alfaKlingon.IndexOf(i[0])) // metodo para ordernar baseado na ordem alfabetica
                       .ThenBy(i => alfaKlingon.IndexOf(i[1]))
                       .ThenBy(i => alfaKlingon.IndexOf(i[2])).ToList();

                }
                if (palavra.Length == 3 && !palavra.Contains('d') && !letrasFoo.Contains(palavra[2])) // faz a contagem das preposições
                {
                    preposicao++;
                }
                else if (palavra.Length >= 8 && letrasFoo.Contains(palavra.Last())) // faz a contagem dos verbos normais e em primeira pessoa
                {
                    verbos++;
                    if (!letrasFoo.Contains(palavra.First()))
                        verbosPrimeiraPessoa++;
                }

                double valorPalavra = 0;
                for (int i = 0; i < palavra.Length; i++) // faz a contagem dos valores das palavras
                {
                    valorPalavra += alfaKlingon.IndexOf(palavra[i]) * Math.Pow(20, i);
                }
                if (valorPalavra >= 440566 && valorPalavra % 3 == 0)// descobre se é um numero bonito
                {
                    numBonitos++;
                }
            }
            ...
            Console.WriteLine("Daniel Santos -- daniel.s.6140@gmail.com\n");
            Console.WriteLine("Teste de Klingon\n");

            Console.WriteLine("-- DADOS DO TEXTO B --");
            Console.WriteLine($"Preposições: {preposicao}");

            Console.WriteLine($"Verbos: {verbos} ");

            Console.WriteLine($"Verbos em Primeira Pessoa: {verbosPrimeiraPessoa} ");

            Console.WriteLine($"Quantidade de Números Bonitos: {numBonitos}\n");

            Console.WriteLine($"Lista do Vocabulário :");
